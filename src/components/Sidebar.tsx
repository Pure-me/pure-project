...
export default function Sidebar({ user, plan = 'solo' }: SidebarProps) {
  ...
  const bottomItems = [
    { href: '/dashboard/billing', label: locale === 'nl' ? 'Abonnement' : 'Subscription', icon: '💳', badge: needsAttention ? '!' : null },
    { href: '/dashboard/account', label: locale === 'nl' ? 'Mijn account' : 'My account', icon: '👤', badge: null },
  ];
  ...
  const NavItem = ({ href, label, icon, badge }: { href: string; label: string; icon: string; badge?: string | null }) => {
    ...
    return (
      <a href={href} className={`sidebar-item${isActive ? ' active' : ''}`} style={{ position: 'relative' }}>
        <span style={{ fontSize: '18px', width: '24px', textAlign: 'center' }}>{icon}</span>
        {label}
        {badge && (
          <span style={{ marginLeft: 'auto', background: '#f87171', color: 'white', fontSize: '10px', fontWeight: '700', width: '16px', height: '16px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            {badge}
          </span>
        )}
      </a>
  ...
  return (
    <>
      ...
      <aside
        ...
        style={{ position: 'fixed', left: 0, top: 0, bottom: 0, width: '260px', background: 'rgba(10,15,30,0.98)', borderRight: '1px solid rgba(255,255,255,0.07)', display: 'flex', flexDirection: 'column', padding: '24px 16px', zIndex: 100, backdropFilter: 'blur(20px)' }}
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '32px', padding: '0 8px' }}>
          <div style={{ width: '40px', height: '40px', background: 'linear-gradient(135deg, #3b82f6, #8b5cf6)', borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '20px', boxShadow: '0 0 20px rgba(59,130,246,0.3)' }}>
            ⬡
          </div>
          <div>
            <div style={{ fontWeight: '800', fontSize: '16px', color: '#f1f5f9', lineHeight: 1.2 }}>Pure Project</div>
            {isExtended ? (
              <div style={{ fontSize: '10px', fontWeight: '700', color: '#a78bfa', letterSpacing: '0.05em', textTransform: 'uppercase' }}>✶ Extended Plan</div>
            ) : (
        ...
        <div style={{ borderTop: '1px solid rgba(255,255,255,0.07)', paddingTop: '12px', marginTop: '16px', display: 'flex', flexDirection: 'column', gap: '4px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '8px', marginBottom: '8px' }}>
            <div style={{ width: '36px', height: '36px', background: 'linear-gradient(135deg, #3b82f6, #8b5cf6)', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '14px', fontWeight: '700', color: 'white', flexShrink: 0 }}>
          ...
          <button
            ...
            style={{ width: '100%', padding: '8px', borderRadius: '8px', background: 'rgba(244,63,94,0.1)', border: '1px solid rgba(244,63,94,0.2)', color: '#f87171', fontSize: '13px', cursor: 'pointer', transition: 'all 0.15s' }}
