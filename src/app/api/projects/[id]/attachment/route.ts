...
import { addProjectAttachment, deleteAttachment, Attachment } from '@/lib/db';
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const att = await addProjectAttachment((await params).id, body);
  if (!att) return NextResponse.json({ error: 'Project niet gevonden' }, { status: 404 });
